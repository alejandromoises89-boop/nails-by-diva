...
<style>
    ...
    .bank-card { background: #fff; padding: 15px; border-radius: 10px; border-left: 4px solid #D4AF37; margin-bottom: 10px; font-size: 0.9rem;}
    ...
    with center_col:
        with st.form("booking_form"):
            ...
            if st.form_submit_button("CONFIRMAR CITA"):
                if name and phone:
                    res = {"id": str(uuid.uuid4())[:6].upper(), "client": name, "service": selected_service, "price": price, "date": str(date), "payment": payment}
...
def success_view():
    ...
    <div style='text-align:center; padding:30px; background:white; border-radius:15px; border:1px solid #D4AF37;'>
        <h2 style='font-family:serif; color:#D4AF37;'>¡RESERVA REGISTRADA!</h2>
        <p>Gracias por elegir <b>Night Speed Diva</b>.</p>
        <p>Tu código de reserva es: <b>{res['id']}</b></p>
    </div>
    """, unsafe_allow_html=True)
    
    if res['payment'] in ["Transferencia", "Pix"]:
        st.markdown("<h4 style='margin-top:20px; text-align:center;'>Información de Pago</h4>", unsafe_allow_html=True)
        c1, c2 = st.columns(2)
        with c1:
            st.markdown('<div class="bank-card"><b>BANCO FAMILIAR</b><br>Cuenta: 815643114<br>Night Speed Diva</div>', unsafe_allow_html=True)
    ...
    st.markdown(f"""
        <a href="{url}" target="_blank" style="text-decoration:none;">
            <div style="background:#25D366; color:white; padding:18px; border-radius:35px; text-align:center; font-weight:bold; font-size:1rem; margin-top:20px;">
